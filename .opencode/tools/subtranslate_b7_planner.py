#!/usr/bin/env python3
"""AUTO-03D B7 batch planning (strictly read-only).

Derives the binding facts of initial batch 7 (``v226-initial-000007``) from
...
file write:
  ...
  * proves plan identity against the recorded R6 request inventory entries,
    including the batch-7 membership anchor;
  ...
  * captures the exact transport payload for plan index 7 through the
...
batch executor, a model, transport, retry, B4, B5 or B6.  Materializing the
...
ACTION_ID = "B7_BATCH_PLANNING"
EXECUTOR_ID = "B7_BATCH_PLANNER_V1"
MODEL = "qwen3.5:9b"
...
LOGICAL_BATCH_ID = "v226-initial-000007"
BATCH_INDEX = 7  # zero-based position inside plan_initial_batches(); the
...
def resolve_source() -> Path:
    for candidate in SOURCE_CANDIDATES:
        ...
        if digest == SOURCE_SHA256:
            return candidate
    raise Blocked("B7_SOURCE_NOT_FOUND")
...
def load_family_facts() -> dict[str, Any]:
    ...
    if facts["source_sha256"] != SOURCE_SHA256:
        raise Blocked("B7_FAMILY_SOURCE_MISMATCH")
    if not revision:
        raise Blocked("B7_EXECUTION_CONTRACT_REVISION_UNBOUND")
    return facts
...
def load_plan_inventory() -> list[dict[str, Any]]:
    ...
    if not isinstance(entries, list) or not entries:
        raise Blocked("B7_PLAN_INVENTORY_INVALID")
...
def capture_payload(pipeline: Any, runner: Any, units: list[Any]) -> dict[str, Any]:
    ...
    try:
        ...
        raise Blocked("B7_PAYLOAD_CAPTURE_FAILED")
    finally:
...
def _build_runner_with_engine(source_path: Path, episode_title: str, engine_root: Path) -> tuple[Any, Any]:
    ...
    runner = V226MemoryRunner(events, profile, config, merged_glossary, None, 3, 79, "b7-planning-read-only")
    ...
    if not original_events:
        raise Blocked("B7_ORIGINAL_EVENTS_UNAVAILABLE")
...
def plan() -> dict[str, Any]:
    ...
    try:
        ...
        if len(packed) != CANONICAL_PLAN_TOTAL:
            raise Blocked(f"B7_PLAN_TOTAL_MISMATCH:packed={len(packed)}:canonical={CANONICAL_PLAN_TOTAL}")
        if len(packed) < len(inventory):
            raise Blocked(f"B7_PLAN_SHORTER_THAN_INVENTORY:packed={len(packed)}:recorded={len(inventory)}")
        ...
        for entry in inventory:
            ...
            if len(ids) != int(entry["event_count"]):
                raise Blocked(f"B7_PLAN_EVENT_COUNT_MISMATCH:index{index}")
            if membership_sha256(ids) != entry["event_id_set_sha256"]:
                raise Blocked(f"B7_PLAN_MEMBERSHIP_MISMATCH:index{index}")
        ...
        for index, recorded in sorted(PAYLOAD_ORACLE_RECORDED_BATCHES.items()):
            if index >= len(packed):
                raise Blocked(f"B7_PAYLOAD_ORACLE_INDEX_OUT_OF_RANGE:{index}")
            ids = batch_ids(packed[index])
            if ids != recorded["unit_ids"]:
                raise Blocked(f"B7_PAYLOAD_ORACLE_UNIT_IDS_MISMATCH:index{index}")
        ...
        return {
            ...
            "transport": False,
            "runtime_write": False,
            "next_gate": "B7_DOCUMENTAL_AUTHORIZATION_REQUIRED",
        }
    finally:
