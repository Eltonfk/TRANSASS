#!/usr/bin/env python3
"""Action-specific, fail-closed executor for the single B5 batch call.
...
The batch-5-specific facts (family_id, episode_id, unit_ids,
...
it the executor is BLOCKED before any backup or network activity.  B5
...
BACKUP_ROOT = BACKUP_PARENT / "subtranslate-auto03d-b5-batch-execution-r1"

ACTION_ID = "B5_BATCH_EXECUTION"
EXECUTOR_ID = "B5_BATCH_EXECUTOR_V1"
MODEL = "qwen3.5:9b"
...
TIMEOUT_SECONDS = 240
# V238 pipeline-level constants (same values as the B4 recovery executor; they
...
AUTHORIZATION_KEY = "auto03d_b5_batch_execution_authorization_r1"
AUTHORIZED_NEXT_ACTION = "B5_BATCH_EXECUTION_AUTHORIZED"
...
# the canonical authorization object before any B5 execution.
...
def current_toolchain_fingerprint() -> str:
    manifest = []
    for relative in B5_TOOLCHAIN_COMPONENTS:
...
def authorization() -> dict[str, Any]:
    ...
    if not isinstance(record, dict):
        raise ExecutionBlocked("B5_EXECUTION_AUTHORIZATION_ABSENT")
    required = {
        "action_id": ACTION_ID,
    ...
    for key, value in required.items():
        if record.get(key) != value:
            raise ExecutionBlocked(f"B5_AUTHORIZATION_CONTRACT_MISMATCH:{key}")
    for key in REQUIRED_FROM_AUTHORIZATION:
        if key not in record:
            raise ExecutionBlocked(f"B5_AUTHORIZATION_MISSING_FACT:{key}")
    if record.get("logical_batch_id") != B5_LOGICAL_BATCH_ID:
        raise ExecutionBlocked("B5_AUTHORIZATION_LOGICAL_BATCH_MISMATCH")
    if state.get("next_action") != AUTHORIZED_NEXT_ACTION:
        raise ExecutionBlocked("B5_AUTHORIZATION_POINTER_MISMATCH")
    fingerprint = record.get("execution_toolchain_fingerprint")
    if not isinstance(fingerprint, str) or len(fingerprint) != 64:
        raise ExecutionBlocked("B5_AUTHORIZATION_TOOLCHAIN_UNBOUND")
    if fingerprint != current_toolchain_fingerprint():
        raise ExecutionBlocked("B5_AUTHORIZATION_TOOLCHAIN_CHANGED")
    snapshot = record.get("snapshot_fingerprint")
    if not isinstance(snapshot, str) or len(snapshot) != 64:
        raise ExecutionBlocked("B5_AUTHORIZATION_SNAPSHOT_UNBOUND")
    return record
...
def create_backup(auth: dict[str, Any]) -> dict[str, str]:
    if BACKUP_ROOT.exists() or BACKUP_ROOT.is_symlink():
        raise ExecutionBlocked("B5_BACKUP_ROOT_ALREADY_EXISTS")
    ...
    for name, source in (("request_payload.json", payload_path),):
        ...
        if digest(destination.read_bytes()) != digest(source.read_bytes()):
            raise ExecutionBlocked("B5_BACKUP_HASH_MISMATCH")
...
def validate_translation(value: Any, unit_ids: list[int]) -> tuple[dict[str, Any], bool]:
    ...
    if [row["id"] for row in projected] != unit_ids:
        raise ExecutionBlocked("B5_RESPONSE_MEMBERSHIP_INVALID")
...
def execute() -> dict[str, Any]:
    ...
    if state.get("state") != "REQUEST_DURABLE":
        raise ExecutionBlocked("B5_REQUEST_NOT_DURABLE")
    with call.exclusive_transport_claim() as owner:
        if not owner:
            raise ExecutionBlocked("B5_TRANSPORT_ALREADY_CLAIMED")
    ...
    if int(response.status_code) != 200:
        raise ExecutionBlocked(f"B5_HTTP_STATUS:{response.status_code}")
    try:
        envelope = json.loads(raw.decode("utf-8"))
        content = (envelope.get("message") or {}).get("content") if isinstance(envelope, dict) else None
        if not isinstance(content, str):
            raise ExecutionBlocked("B5_RESPONSE_CONTENT_MISSING")
