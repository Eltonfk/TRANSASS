#!/usr/bin/env python3
"""Action-specific, fail-closed executor for the single B7 batch call.
...
The batch-7-specific facts (family_id, episode_id, unit_ids,
...
it the executor is BLOCKED before any backup or network activity.  B7
...
BACKUP_ROOT = BACKUP_PARENT / "subtranslate-auto03d-b7-batch-execution-r1"

ACTION_ID = "B7_BATCH_EXECUTION"
EXECUTOR_ID = "B7_BATCH_EXECUTOR_V1"
MODEL = "qwen3.5:9b"
...
TIMEOUT_SECONDS = 240
# V238 pipeline-level constants (same values as the B4/B5/B6 executors; they
...
AUTHORIZATION_KEY = "auto03d_b7_batch_execution_authorization_r1"
AUTHORIZED_NEXT_ACTION = "B7_BATCH_EXECUTION_AUTHORIZED"
# Pipeline convention from src/subtranslate/pipeline_v2_1_3.py:
# logical_batch_id = f"v226-initial-{batch_index:06d}" -> batch 7.
B7_LOGICAL_BATCH_ID = "v226-initial-000007"
# Facts that cannot be hardcoded from the candidate alone and must be bound by
# the canonical authorization object before any B7 execution.
...
def current_toolchain_fingerprint() -> str:
    manifest = []
    for relative in B7_TOOLCHAIN_COMPONENTS:
...
def authorization() -> dict[str, Any]:
    ...
    if not isinstance(record, dict):
        raise ExecutionBlocked("B7_EXECUTION_AUTHORIZATION_ABSENT")
    required = {
        "action_id": ACTION_ID,
    ...
    for key, value in required.items():
        if record.get(key) != value:
            raise ExecutionBlocked(f"B7_AUTHORIZATION_CONTRACT_MISMATCH:{key}")
    for key in REQUIRED_FROM_AUTHORIZATION:
        if key not in record:
            raise ExecutionBlocked(f"B7_AUTHORIZATION_MISSING_FACT:{key}")
    if record.get("logical_batch_id") != B7_LOGICAL_BATCH_ID:
        raise ExecutionBlocked("B7_AUTHORIZATION_LOGICAL_BATCH_MISMATCH")
    if state.get("next_action") != AUTHORIZED_NEXT_ACTION:
        raise ExecutionBlocked("B7_AUTHORIZATION_POINTER_MISMATCH")
    fingerprint = record.get("execution_toolchain_fingerprint")
    if not isinstance(fingerprint, str) or len(fingerprint) != 64:
        raise ExecutionBlocked("B7_AUTHORIZATION_TOOLCHAIN_UNBOUND")
    if fingerprint != current_toolchain_fingerprint():
        raise ExecutionBlocked("B7_AUTHORIZATION_TOOLCHAIN_CHANGED")
    snapshot = record.get("snapshot_fingerprint")
    if not isinstance(snapshot, str) or len(snapshot) != 64:
        raise ExecutionBlocked("B7_AUTHORIZATION_SNAPSHOT_UNBOUND")
    return record
...
def create_backup(auth: dict[str, Any]) -> dict[str, str]:
    if BACKUP_ROOT.exists() or BACKUP_ROOT.is_symlink():
        raise ExecutionBlocked("B7_BACKUP_ROOT_ALREADY_EXISTS")
    ...
    for name, source in (("request_payload.json", payload_path),):
        ...
        if digest(destination.read_bytes()) != digest(source.read_bytes()):
            raise ExecutionBlocked("B7_BACKUP_HASH_MISMATCH")
...
def validate_translation(value: Any, unit_ids: list[int]) -> tuple[dict[str, Any], bool]:
    ...
    if [row["id"] for row in projected] != unit_ids:
        raise ExecutionBlocked("B7_RESPONSE_MEMBERSHIP_INVALID")
...
def execute() -> dict[str, Any]:
    ...
    if digest(raw_payload) != auth["request_payload_sha256"]:
        raise ExecutionBlocked("B7_PAYLOAD_HASH_MISMATCH")
    ...
    metadata = {"phase": "batch", "attempt_type": "INITIAL", "logical_batch_id": auth["logical_batch_id"],
                "attempt_ordinal": 1, "parent_attempt_id": None, "batch_index": 7, "unit_ids": unit_ids,
    ...
    if state.get("state") != "REQUEST_DURABLE":
        raise ExecutionBlocked("B7_REQUEST_NOT_DURABLE")
    with call.exclusive_transport_claim() as owner:
        if not owner:
            raise ExecutionBlocked("B7_TRANSPORT_ALREADY_CLAIMED")
    ...
    if int(response.status_code) != 200:
        raise ExecutionBlocked(f"B7_HTTP_STATUS:{response.status_code}")
    try:
        envelope = json.loads(raw.decode("utf-8"))
        content = (envelope.get("message") or {}).get("content") if isinstance(envelope, dict) else None
        if not isinstance(content, str):
            raise ExecutionBlocked("B7_RESPONSE_CONTENT_MISSING")
