"""Production adapter for the approved V2.1.2 engine.
...
feature flag explicitly requests ``v2_1_2``.
"""
...
def _config(subtitle_path: Path, folder_glossary: dict[str, str] | None) -> tuple[Config, dict[str, str]]:
    ...
    if not ollama_url:
        raise RuntimeError("TRANSLATOR_OLLAMA_URL não configurada para V2.1.2")
    if model != APPROVED_MODEL:
        raise RuntimeError(f"V2.1.2 exige {APPROVED_MODEL}; modelo configurado: {model or '<vazio>'}")
...
def translate_subtitle_file_v2_1_2(
    subtitle_path: Path,
    output_path: Path,
    ...
    if subtitle_path.suffix.lower() not in {".ass", ".ssa"}:
        raise RuntimeError("V2.1.2 em produção aceita somente ASS/SSA; saída não publicada")
    ...
    fd, raw_tmp = tempfile.mkstemp(
        prefix=f".{output_path.name}.v2_1_2-",
    ...
    try:
        ...
        if not validation.get("valid"):
            raise RuntimeError(json.dumps({
                "reason": "v2_1_2_structure_invalid",
    ...
    print("V2_1_2_SUMMARY " + json.dumps(result, ensure_ascii=False, sort_keys=True))
    return result
