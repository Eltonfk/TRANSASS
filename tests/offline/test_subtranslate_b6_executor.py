"""Offline deterministic tests for the AUTO-03D B6 batch executor.

Focus: contract shape, B6-only bindings, fail-closed behavior without a
canonical authorization object (blocks before any backup or network), and
response validation semantics.  No network, no model, no writes.
"""
...
B6_UNIT_IDS = [58, 59, 60, 61, 62, 63, 64, 65]


def _load_tool():
    spec = importlib.util.spec_from_file_location("subtranslate_b6_executor", TOOL_PATH)
...
def test_plan_contract_without_authorization(executor):
    ...
    assert result["action_id"] == "B6_BATCH_EXECUTION"
    assert result["executor_id"] == "B6_BATCH_EXECUTOR_V1"
...
def test_apply_fail_closed_without_authorization(executor, tmp_path, monkeypatch):
    ...
    with pytest.raises(executor.ExecutionBlocked) as excinfo:
        executor.execute()
    assert "B6_EXECUTION_AUTHORIZATION_ABSENT" in str(excinfo.value)


def test_bindings_are_b6_exclusive():
    source = TOOL_PATH.read_text(encoding="utf-8")
    assert 'AUTHORIZATION_KEY = "auto03d_b6_batch_execution_authorization_r1"' in source
    assert 'AUTHORIZED_NEXT_ACTION = "B6_BATCH_EXECUTION_AUTHORIZED"' in source
    assert 'B6_LOGICAL_BATCH_ID = "v226-initial-000006"' in source
    assert "auto03d_b5" not in source
    assert "B5_BATCH_EXECUTION_AUTHORIZED" not in source
    assert '"batch_index": 6' in source
...
def test_validate_translation_accepts_exact_membership(executor):
    value = {"translations": [{"id": uid, "text": f"texto {uid}"} for uid in B6_UNIT_IDS]}
    projected, normalized = executor.validate_translation(value, B6_UNIT_IDS)
    assert normalized is False
    assert [row["id"] for row in projected["translations"]] == B6_UNIT_IDS


def test_validate_translation_rejects_wrong_membership(executor):
    swapped = list(reversed(B6_UNIT_IDS))
    value = {"translations": [{"id": uid, "text": f"texto {uid}"} for uid in swapped]}
    with pytest.raises(executor.ExecutionBlocked) as excinfo:
        executor.validate_translation(value, B6_UNIT_IDS)
    assert "B6_RESPONSE_MEMBERSHIP_INVALID" in str(excinfo.value)


def test_validate_translation_rejects_cardinality(executor):
    value = {"translations": [{"id": uid, "text": "x"} for uid in B6_UNIT_IDS[:-1]]}
    with pytest.raises(executor.ExecutionBlocked) as excinfo:
        executor.validate_translation(value, B6_UNIT_IDS)
    assert "B6_RESPONSE_CARDINALITY_INVALID" in str(excinfo.value)


def test_validate_translation_flags_extra_properties(executor):
    rows = [{"id": uid, "text": f"t{uid}", "extra": 1} for uid in B6_UNIT_IDS]
    projected, normalized = executor.validate_translation({"translations": rows}, B6_UNIT_IDS)
...
def test_cli_plan_smoke(executor, capsys):
    ...
    assert captured["action_id"] == "B6_BATCH_EXECUTION"
