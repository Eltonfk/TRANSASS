"""Offline deterministic tests for the AUTO-03D B6 batch planner.

The central proof mirrors the B5 planner: the tool must reconstruct the
canonical 233-batch plan (membership anchors recorded in the R6 request
inventory, including the batch-6 entry) and reproduce the executed R6C
payloads byte-exactly before its batch-6 derivation is trustworthy.
...
def _load_tool():
    spec = importlib.util.spec_from_file_location("subtranslate_b6_planner", TOOL_PATH)
...
def test_plan_is_ready_with_complete_contract(plan):
    ...
    target = plan["target"]
    assert target["logical_batch_id"] == "v226-initial-000006"
    assert target["batch_index"] == 6
...
def test_target_membership_matches_recorded_inventory(plan):
    ...
    # Distinct from the batch-5 anchor guards against copy-paste drift.
...
def test_fail_closed_when_source_missing(planner, monkeypatch):
    ...
    with pytest.raises(planner.Blocked) as excinfo:
        planner.plan()
    assert "B6_SOURCE_NOT_FOUND" in str(excinfo.value)
...
def test_fail_closed_when_batch_index_out_of_range(planner, monkeypatch):
    ...
    with pytest.raises(planner.Blocked) as excinfo:
        planner.plan()
    assert "B6_BATCH_INDEX_OUT_OF_RANGE" in str(excinfo.value)
