"""Offline deterministic tests for the AUTO-03D B7 batch planner.

The central proof mirrors the B5/B6 planners: the tool must reconstruct the
canonical 233-batch plan (membership anchors recorded in the R6 request
inventory, including the batch-7 entry) and reproduce the executed R6C
payloads byte-exactly before its batch-7 derivation is trustworthy.
No network, no model, no writes.
"""
...
def _load_tool():
    spec = importlib.util.spec_from_file_location("subtranslate_b7_planner", TOOL_PATH)
...
def test_plan_is_ready_with_complete_contract(plan):
    ...
    target = plan["target"]
    assert target["logical_batch_id"] == "v226-initial-000007"
    assert target["batch_index"] == 7
...
def test_target_membership_matches_recorded_inventory(plan):
    ...
    # Distinct from the batch-5/6 anchors guards against copy-paste drift.
...
def test_fail_closed_when_source_missing(planner, monkeypatch):
    ...
    with pytest.raises(planner.Blocked) as excinfo:
        planner.plan()
    assert "B7_SOURCE_NOT_FOUND" in str(excinfo.value)
...
def test_fail_closed_when_batch_index_out_of_range(planner, monkeypatch):
    ...
    with pytest.raises(planner.Blocked) as excinfo:
        planner.plan()
    assert "B7_BATCH_INDEX_OUT_OF_RANGE" in str(excinfo.value)
