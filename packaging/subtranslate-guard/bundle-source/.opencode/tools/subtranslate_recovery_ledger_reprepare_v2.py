#!/usr/bin/env python3
"""AUTO-03B2B: bundle-isolated recovery-ledger re-preparation executor.

This source preserves the V1 ledger semantics while deriving its source root
from ``__file__``.  It is source for a future installed bundle only; it is not
registered as a live OpenCode tool in this phase.
"""
...
_EXECUTOR_FILE = Path(__file__)
if _EXECUTOR_FILE.is_symlink():
    raise RuntimeError("EXECUTOR_SOURCE_SYMLINK")
BUNDLE_ROOT = _EXECUTOR_FILE.resolve().parents[2]
...
BACKUP_ROOT = Path("/var/lib/subtranslate-guard/backups")
...
PARENT_EXECUTOR_SHA256 = "2f0fc420399671f06040a46405d42eca532c692d0b62729353fb90b840a04801"
...
FIXED_INTERPRETER = "/usr/bin/python3.12"
V2_RELATIVE_EXECUTOR = ".opencode/tools/subtranslate_recovery_ledger_reprepare_v2.py"
FIXED_APPLY_ARGV = (FIXED_INTERPRETER, "-I", "-B", "<protected-release>/" + V2_RELATIVE_EXECUTOR, "--apply")

try:
    if BUNDLE_ROOT.is_symlink() or not BUNDLE_ROOT.is_dir():
        raise RuntimeError("BUNDLE_ROOT_UNSAFE")
    source_root = (BUNDLE_ROOT / "src").resolve(strict=True)
    if source_root.parent != BUNDLE_ROOT.resolve(strict=True):
        raise RuntimeError("BUNDLE_SOURCE_ROOT_ESCAPE")
...
class RecoveryLedgerExecutor:
    ...
    def _base_plan(self, *, current_target_sha256: str | None, blockers: list[dict[str, Any]], unknowns: list[dict[str, Any]], expected: list[dict[str, Any]] | None = None) -> dict[str, Any]:
        ...
        return {
            ...
            "parent_executor_sha256": PARENT_EXECUTOR_SHA256,
            "executor_version": EXECUTOR_VERSION,
            "bundle_root_derivation": "__file__.resolve().parents[2]",
            "fixed_apply_argv": list(FIXED_APPLY_ARGV),
            "backup_root_policy": str(BACKUP_ROOT),
